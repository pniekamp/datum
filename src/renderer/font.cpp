...
void ResourceManager::request<Font>(DatumPlatform::PlatformInterface &platform, Font const *font)
{
  ...
  if (slot->state.compare_exchange_strong(empty, Font::State::Loading))
  {
    if (auto asset = slot->asset)
    {
      ...
      {       
        ...
        slot->sheet = create<Texture>(assets()->find(asset->id + glyphs), Texture::Format::RGBA);

        if (slot->sheet)
        {
          float sx = 1.0f / slot->sheet->width;
          float sy = 1.0f / slot->sheet->height;
    ...
    slot->state = (slot->texcoords && slot->alignment && slot->dimension && slot->advance && slot->sheet) ? Font::State::Waiting : Font::State::Empty;
  }
  ...
  {
    request(platform, slot->sheet);

    slot->state = (slot->sheet->ready()) ? Font::State::Ready : Font::State::Waiting;
  }
}
